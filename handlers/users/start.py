...
from handlers.users.vocabulary_words import language_choice, welcome, orientation, types_direction, casting_title_text, \
    casting_title, casting_open, casting_open_text
...
@dp.callback_query_handler(
    state=[Personaldata.Welcome.wel, Personaldata.PaymeData.pay_data, Personaldata.PersonalInfo.info,
           Personaldata.PersonalPhoto.photo], text='tekshirish')
async def bot_start_call(call_msg: types.CallbackQuery):
    call = call_msg
...
async def send_start_lan(user_id):
    try:
        ...
        if language_request['language_db'] is None or language_request['language_db'] == "uz":
            ...
            await dp.bot.send_message(chat_id=user_id, text=f"<b>{language_choice['uz']}</b>",
            ...
            inline_ru = InlineKeyboardButton(f'🇷🇺 Rus', callback_data=f"ru")
            inline_keyboard_language.add(inline_uz, inline_ru)
            await dp.bot.send_message(chat_id=user_id, text=f"<b>{language_choice['ru']}</b>",
                                      reply_markup=inline_keyboard_language)
    except TypeError:
        inline_keyboard_language = InlineKeyboardMarkup(row_width=2)
        inline_uz = InlineKeyboardButton(f"🇺🇿 O'zbek", callback_data=f"uz")
...
async def send_welcome_message(user_id, locale):
    inline_keyboard_send_welcome_message = InlineKeyboardMarkup(row_width=1)
    inline_uz1123 = InlineKeyboardButton(f"{casting_title_text[locale]}", url=f"{casting_title[locale]}")
    inline_uz3213 = InlineKeyboardButton(f"{casting_open_text[locale]}",
                                         url=f'{casting_open[locale]}')
    inline_keyboard_send_welcome_message.add(inline_uz1123, inline_uz3213)
    await dp.bot.send_message(chat_id=user_id, text=welcome[locale],
                              reply_markup=inline_keyboard_send_welcome_message)
